...
### Web Production
Generates a static optimized bundle in the `dist` folder.
```bash
npx expo export:web
```

### Desktop Installers (Tauri)
Tauri packages the web build into native installers.
...
> [!TIP]
> Ensure `frontendDist` in `tauri.conf.json` points to the `dist` folder generated by the Expo web export.
